...
//global pointers
void* libhandle;
struct main_table* (*d_create_table)(int size);
struct file_info* (*d_merge_pair)(struct files_pair* files);
struct block* (*d_create_block)(struct file_info* temp_file);
int (*d_add_block)(struct main_table* table, struct block* blk);
int (*d_merge_and_add)(struct main_table* table, struct files_pair* pair);
void (*d_merge_files)(struct main_table* table, struct sequence* seq);
int (*d_number_of_rows)(struct block* blk);
int (*d_rows_in_block)(struct main_table* table, int index);
void (*d_remove_block)(struct main_table* table, int index);
void (*d_remove_row_from_block)(struct main_table* table, int block_index, int row);
void (*d_print_block)(struct main_table* table, int index);
void (*d_print_all)(struct main_table* table);
struct sequence* (*d_create_sequence)(int size);
void (*d_free_main_table)(struct main_table* table);
...
void pass_test(struct test* test){
    ...
    for(int i = 0; i < test->number_of_cases; i+=1){
        ...
        d_free_main_table(table); //for security
        table = d_create_table(test->numbers_of_blocks[i]);
        ...
        for (int j = 0; j<test->numbers_of_blocks[i]; j+=1){
            merged_files[j] = d_merge_pair(test->seq->pairs[j%test->seq->size]);
        }
        ...
        real_times[2] = clock();
        for (int j = 0; j<test->numbers_of_blocks[i]; j+=1){
            struct block* blk = d_create_block(merged_files[j]);
            d_add_block(table, blk);
        }
        ...
        for (int j = 0; j<100; j+=1){
            struct block* blk = d_create_block(merged_files[j%test->numbers_of_blocks[i]]);
            d_add_block(table, blk);
            d_remove_block(table, 0);
        }
...
int main(int argc, char** argv){
    ...
    libhandle = dlopen("./libsharedmerge.so", RTLD_LAZY);
    if(libhandle == NULL){
    ...
    d_create_table = dlsym(libhandle, "create_table");
    d_merge_pair = dlsym(libhandle, "merge_pair");
    d_create_block = dlsym(libhandle, "create_block");
    d_add_block = dlsym(libhandle, "add_block");
    d_merge_and_add = dlsym(libhandle, "merge_and_add");
    d_merge_files = dlsym(libhandle, "merge_files");
    d_number_of_rows = dlsym(libhandle, "number_of_rows");
    d_rows_in_block = dlsym(libhandle, "rows_in_block");
    d_remove_block = dlsym(libhandle, "remove_block");
    d_remove_row_from_block = dlsym(libhandle, "remove_row_from_block");
    d_print_block = dlsym(libhandle, "remove_row_from_block");
    d_print_all = dlsym(libhandle, "print_all");
    d_create_sequence = dlsym(libhandle, "create_sequence");
    d_free_main_table = dlsym(libhandle, "free_main_table");
    ...
    for(int i = 1; i < argc; i+=1){
        ...
        if (strcmp(argv[i], "create_table") == 0){
            ...
            if (size <= 0) return -1;
            table = d_create_table(size);
        }
        else if (strcmp(argv[i], "merge_files") == 0){
            i += 1;
            int not_null_id = 0;
            seq = d_create_sequence(table->size);
            ...
            d_merge_files(table, seq);
            i-=1;
        }
        else if(strcmp(argv[i], "remove_block") == 0){
            i += 1;
            int id = atoi(argv[i]);
            d_remove_block(table, id);
        }
            ...
            i += 1;
            int row = atoi(argv[i]);
            d_remove_row_from_block(table, block, row);
        }
        else if(strcmp(argv[i], "print_block") == 0){
            i += 1;
            int id = atoi(argv[i]);
            d_print_block(table, id);
        }
